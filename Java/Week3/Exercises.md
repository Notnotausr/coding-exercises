...
2. Create a separate class with a `main` method where you create **multiple Character objects** and store them in an **array**. Use a **loop** to go through the array and print each character’s stats.
3. Add a **method that changes the character’s health** (for example, taking damage or healing). Use **conditionals (`if` / `else`)** to check things like whether a character’s health drops to 0 (the character is defeated) or if their level increases after gaining enough experience.
4. Write a loop that simulates several turns of the game, updating and printing the characters’ stats each turn.
